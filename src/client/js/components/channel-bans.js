...
export default class ChannelBans extends React.Component {
  constructor () {
    super()
  ...
  componentDidMount () {
    ...
    client.on('367', (sender, dest, message) => {
      bans.push({
        mask: message[2],
        setBy: message[3],
        time: message[4]
    ...
    client.on('368', (sender, dest, message) => {
      if (!bans.length) {
        bans.push(null)
      }
  ...
  update () {
    client.send('MODE', this.props.params.channel, 'b')
  }

  componentWillUnmount () {
    client.removeAllListeners('367')
    client.removeAllListeners('368')
  }

  remove (masks) {
    client.send(`CS MODE ${this.props.params.channel} SET -${'b'.repeat(masks.length)} ${masks.join(' ')}`)
